...
class MyMainHoney(threading.Thread):
	...
	def rx(self):
		...
		if('Yes' == self.honeydb['enabled']):
			t =  datetime.datetime.now()
			lib.honeydb.logger(self.honeydb['url'], self.honeydb['secret'], t.strftime("%Y-%m-%d"), t.strftime("%H:%M:%S"), t.strftime("%Y-%m-%d") + " " + t.strftime("%H:%M:%S"), t.microsecond, 'RX', self.host, self.port, "[" + self.service + "]", self.remote_host, self.remote_port, data.encode("hex"))
