...
def post_sumologic(useragent, url, date, time, date_time, millisecond, session, protocol, event, local_host, local_port, service, remote_host, remote_port, data):
	h = hashlib.md5()
	h.update(data)
	...
	try:
		r       = requests.post(url, headers=headers, json=data, verify=False, timeout=3)
		page    = r.text
